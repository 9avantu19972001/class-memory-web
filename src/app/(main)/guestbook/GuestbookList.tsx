...
import { deleteGuestbookEntry, togglePinGuestbookEntry, toggleGuestbookReaction } from './actions'
...
export default function GuestbookList({
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    if (activeTab === 'received') {
      const isReceived = entry.visibility === 'selected' && entry.allowed_user_ids?.includes(currentUserId || '') && entry.user_id !== currentUserId
    ...
    const authorMatch = entry.author?.full_name?.toLowerCase().includes(term) || entry.author?.nickname?.toLowerCase().includes(term)
  ...
  return (
    <div>
      ...
      {/* Grid of Nostalgic Sticky Notes */}
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
        {filteredEntries.map((entry, idx) => {
          ...
          return (
            <div
              key={entry.id}
              className={`break-inside-avoid relative rounded-2xl p-6 border shadow-md hover:shadow-xl transition-all duration-300 hover:rotate-0 hover:scale-[1.02] flex flex-col group ${colorConfig.bg} ${colorConfig.border} ${colorConfig.text} ${rotClass}`}
            >
              ...
              {/* Note Footer: Author Info, Date, Likes & Actions */}
              <div className="flex items-center justify-between gap-3 text-xs">
                ...
                {/* Right Actions: Likes & Admin buttons */}
        ...
        {filteredEntries.length === 0 && (
          <div className="col-span-full py-16 text-center bg-card rounded-2xl border border-dashed border-border p-8">
            <div className="w-12 h-12 rounded-full bg-secondary/50 flex items-center justify-center mx-auto mb-3 text-2xl">
              📝
            </div>
